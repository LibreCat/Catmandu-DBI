...
sub add_row {
    ...
    if( $mapping->{jsonb} && $dbh->{pg_server_version} < 90400 ) {
        Catmandu::Error->throw( "JSONB data type only supported in postgres 9.4 and above" );
    }
    my $binary_type = $mapping->{jsonb} ? DBD::Pg->PG_JSONB : DBD::Pg->PG_BYTEA;
    my $id     = $row->{$id_col};
    my @cols   = keys %$row;
